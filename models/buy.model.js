...
class BuyModel {
    ...
    insertPurchaseTrx = async (trxData) => {
        let sql = `INSERT INTO transactions(user_id, bnb_address, amount,fee, token,usd_amount, withdrable, phase, transactionHash, locking_duration,user_transaction_id,image ) VALUES( '${trxData.user_id}'  , '${trxData.bnb_address}', '${trxData.amount}','${trxData.fee}', '${trxData.token}', '${trxData.usd_amount}', '${trxData.withdrable}', '${trxData.phase}', '${trxData.transactionHash}' , '${trxData.locking_duration}','${trxData.transaction_id}','${trxData.image}') `;
    ...
    getUserDetails = async (id) => {
        let sql = `SELECT u.id,u.email,u.balance, u.vesting_balance, u.bnb_address, r.id as refid,r2.id as refid2, u.refer_by FROM registration u LEFT JOIN registration as r ON u.referred_by_id = r.id left join registration as r2 on r2.id=r.referred_by_id where u.id = '${id}' `;
        const [result, fields] = await promisePool.query(sql);
